...
""" LIST RECORDS """
def handler_list_records(metadataPrefix):
    if metadataPrefix is None: #no metadataPrexif to test SPARQL results with default namespace
        return handler_list_records_no_mp()
    else:
        return handler_list_records_mp(metadataPrefix) 
...
def handler_list_records_mp(metadataPrefix: str):
    if metadataPrefix==OAI_DATACITE_PF:
...
def list_records_oai_datacite_1():
    ...
    # loop over records
    i=1
    listRecords = ET.SubElement(root, "ListRecords")
...
""" GET RECORD """
def handler_get_record(identifier: str,metadataPrefix: str):
    # metadataPrefix logic
    if metadataPrefix is None: #no metadataPrexif to test SPARQL results with default namespace
        return handler_get_record_no_mp(identifier)
    else:
        return handler_get_record_mp(identifier, metadataPrefix)     
...
def handler_get_record_mp(identifier: str, metadataPrefix: str):
    if metadataPrefix==OAI_DATACITE_PF:
...
def list_metadata_formats(args):
    ...
    SubElement(format_el, f"{{{OAI_NS}}}schema").text = OAI_DATACITE_SCHEMA
    SubElement(format_el, f"{{{OAI_NS}}}metadataNamespace").text = OAI_DATACITE_NS
